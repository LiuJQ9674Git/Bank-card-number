...
def get_img(img, file):
    ...
    # 2.二值化
    ret, img = cv2.threshold(blur, 60, 255, cv2.THRESH_BINARY)
    ...
    for m in range(len(Position)):
        if Position[m][3]/(Position[m][2]-Position[m][0]) > 1 \
                and Position[m][3]/(Position[m][2]-Position[m][0]) < 5:
            temp_img = img[Position[m][1]:Position[m][3],
                       Position[m][0]:Position[m][2]]
